...
void inicializa_bot(Bot *bot, const char *nome, const char *equipe,
                    FuncaoDecisao f) {
  bot->id = id_atual++;
  snprintf(bot->nome, sizeof(bot->nome), "%s", nome);
  snprintf(bot->equipe, sizeof(bot->equipe), "%s", equipe);
  bot->vida = VIDA_MAX, bot->energia = ENERGIA_INICIAL, bot->curas = NUM_CURAS,
  bot->contra_ataques = NUM_CONTRA_ATAQUES, bot->tomar_decisao = f;
...
void gerar_relatorio_bots(Bot **bots, size_t num_bots) {
  remove("bot_stats.csv");
  FILE *relatorio = fopen("bot_stats.csv", "w");
  if (!relatorio)
    printf("ERRO: NAO FOI POSSIVEL GERAR RELATORIO");
...
void gerar_header_CSV(FILE *relatorio) {
  fputs("nome, equipe, pontuacao, abates, mortes, vitorias, derrotas, empates, "
        "dano_tomado, bons_CA, boas_DEF, decisoes, tempo_decisao\n",
        relatorio);
}
